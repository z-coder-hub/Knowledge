...
# 四、接口
# 五、类
> TS中的类，拥有和Java、C#、C++等一些编程语言的描述符
> 例如：
> 1. public属于公共的，也就是说这个修饰符定一个方法或者属性不管是在类的内部还是外部都可以被调用
> 2. private属于私有的，这个描述符指定的方法或者属性只可以在类的本身调用
> 3. protected属于继承描述符，可以在继承的类中调用父类的方法或者属性
这些描述符在初始化成员的时候可以省略初始化
``` ts
class Parent {
  constructor(public name: string) {}
  // 不需要在{}中再次使用this.name = name
  // 这三个描述符都有这个功能
}
```
